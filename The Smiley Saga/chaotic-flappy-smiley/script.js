...
let dead = false;
let score = 0;
let highScore = parseInt(localStorage.getItem('chaoticFlappyHighScore')) || 0;
...
function mainLoop() {
    ...
    if (!dead) {
        ...
        if (Math.floor(score / 10) > highScore) {
            ...
            localStorage.setItem('chaoticFlappyHighScore', highScore.toString());
            hScoreDisplay.textContent = highScore;
        };
        ...
        // Spawn enemies
        timeUntilSpawn--;
        if (Math.random() < 0.05 && gameStarted) {
            enemies.push({
                x: gameArea.width,
                y: Math.random() * (gameArea.height - 100),
