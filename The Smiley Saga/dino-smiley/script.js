...
window.addEventListener('keydown', (e) => {
    if (e.code === 'Space' && player.y === gameArea.height - player.height) {
        spaceDown = true;
        gameStarted = true;
    }
});
...
function mainLoop() {
    ...
    if (!dead) {
        ...
        if (spaceDown && player.y >= gameArea.height - 120 && !jumped) {
            player.speedY -= 3;
        };
        ...
        // Confine player to the game area
        ...
        for (let enemy of enemies) {
            ...
            // Slightly rotate enemy
            enemy.rotation -= enemy.speedX * -0.01;
